...
**And the number in the third column is not the number the chooser sees.** `RegisterChoice.preference` returns registers in order and `Realizer.realize` takes the first one it can fill, so a stage bench of 60 split three ways is three benches of 20 and only one of them is ever offered. `quietDay` stage 1 holds 67 lines and the run reached the 21 plain ones across 212 firings. **Measured where the choice is made, phase 9's hot benches are warm.** Either this column is restated per register or the realizer chooses across the registers it can fill; both are outside authoring, and until one of them happens a sixty line stage bench buys twenty lines of variety. Section 12 carries the measurement.

Current, counted rather than estimated: **1,775 Pulse lines producing 291,807 surfaces, 2,200 Report and guidance lines producing roughly 24,900, and 758 Momentum and banner lines producing 3,277.** Combined, 4,733 authored lines and roughly 320,000 surfaces. **Every figure here is now recounted from the file rather than owed.** Two of them were left open on purpose and both have since landed: volume 3 closed while volume 1 was mid growth, its Pulse line count was 1,380 and climbing at the moment that sentence was written, and a figure recorded then would have been wrong before it was read. The Report line count was the last of them, and it moved from 804 to 1,617 in the pass that grew the ten hottest Report families; `CORPUS_2_REPORT.md` states the same figure in six places and all of them moved together, which is why they belonged to one pass over that volume rather than to a second hand correction here. Storage is not a consideration; quality is the only constraint. All three volumes now have their hot families inside the band in the table above. Volume 2 closed last, taking its final nine hot benches from 8 to 18 lines each to 60 and moving from 1,617 lines to 2,200, which is the last movement any figure in this paragraph is owed. **The 1,874 this sentence held before that pass was never a count of anything**: it was written when the Report file carried 1,617, and `CorpusTotalsAuditTest` had been failing on it, here and in `CORPUS_3_MOMENTUM.md`, from the moment it was written. A projected total is the same defect as a stale one and the audit does not distinguish them.
...
The readings the deferred checks produce are the baseline phase 9 is judged against and the full tables, with the reasoning, are in `DECISIONS.md` and `docs/BUILD_STATE.md`. **Seven measurements exist and the seventh is the current one.** Phase 5, the facts phase, the slot bindings phase, the rules pass, the pass that repaired the persona set, the pass that built the two family scope gates of `MASTER_BUILD_PROMPT.md` 14b, and phase 9's authoring, over the same eleven personas and the same simulated year each time. The twelfth persona `cyclicalDips` is deliberately outside `SimulationPersona.ALL` and outside every column: every measurement recorded here is quoted against the eleven, and a twelfth would move all of them silently.
...
| reading | target | phase 5 | facts | bindings | rules pass | persona repair | the gates | phase 9 |
|---|---|---|---|---|---|---|---|---|
| Pulse silence, every persona together | 8 to 25 percent of opened days | 76 percent | 73 percent | 68 percent | 68 percent | 63 percent | 65 percent, 65.7 exact | **65 percent, 65.7 exact** |
| Pulse silence, per persona | the same band | 43 to 98 | 42 to 98 | 40 to 97 | 40 to 97 | 37 to 97 | 37 to 97 | **37 to 97, none in band** |
| Pulse families that ever fired | 11 of 11 | 6 of 11 | 7 of 11 | 8 of 11 | 8 of 11 | 11 of 11 | 11 of 11 | **11 of 11** |
| every family the corpus declares fires | every one | not measured | 58 of 78 | 60 of 78 | 65 of 78 | 71 of 78 | 69 of 78 | **70 of 80, and 69 of the same 78** |
| every stage of every hot family fires | all | 29 hot, one gap | 31 hot, two gaps | 33 hot, two gaps | 35 hot, two gaps | 36 hot, one gap | 36 hot, one gap | **36 hot, one gap** |
| no variant repeats inside ninety days | none | 7,384 | 7,430 | 7,445 | 7,376 | 7,418 | 7,370 | **3,898, tightest after 1 day** |
| layer 5 vetoes across the run | none | not reported | not reported | 107, every one check 1 | 0, and 92 absences | 0, and 85 absences | 0, and 38 absences | **0, and 41 absences on purpose** |
| pattern slots, and their concentration | no family holds a section | not reported | not reported | 416 of 419, 8 families, top three 402 | 401 of 419, 12 families, top three 296 | 399 of 419, 13 families, top three 295 | 397 of 419, 12 families, top three 298 | **397 of 419, 12 families, top three 298** |
| no family over a fifth of a year's Pulses | 20 percent | 27 to 60 | 25 to 57 | 25 to 51 | 25 to 51 | 25 to 48 | not recorded here | **25 to 48** |
| no two consecutive report leads share a band | none | 715 | 725 | 716 | 712 | 719 | not recorded here | **277** |
| no three consecutive parallel numeric clauses | none | 27 runs | 37 runs | 36 runs | 41 runs | 41 runs | not recorded here | **121 runs** |
| registers a surface actually reaches | every one authored | not measured | not measured | not measured | not measured | not measured | not measured | **one, on five of the six surfaces** |
...
So the column has two producers, one in the selector and one in the realizer, and neither is sensitive to bench size. 3,230 new lines changing the count by zero is the empirical half of the same statement. **The 1,161 can only be moved by a rule, a binding or a cooldown**, and the owner's standing instruction that this is reported and not ground at is unchanged by that: it is the same conclusion with a mechanism under it.
...
**Which of the two situations this is.** Authoring reached what it could and reached it fully: every hot bench is inside the band, every reading a bench can move improved, and no line phase 9 wrote is unrenderable. Silence is not one of those readings and never was. **Something else is in the way, and after this run it is named rather than suspected**: 1,161 days held back by five filters that do not read the corpus, one register reachable per surface, one section 9 rule with no implementation, and 82 lines waiting on a binding. None of the four is an authoring problem and all four are cheap to state as work.
