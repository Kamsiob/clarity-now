...
| deleteMuted | `#724444` | the delete action, wherever it is offered. Never a saturated red |
...
- **This table said `the delete swipe action only` and the app has never matched it.** The area detail sheet has drawn its Delete row's glyph and label in this token since phase 2, and 10.20's archive row is the third place. The scoping sentence this document repeats about the four function colors is that each carries **one meaning**, so that a second use is never a color meaning two things, and `deleteMuted` means delete at all three. **The word that was wrong was `swipe`, not `only`**, and it is corrected here rather than in each call site, because the alternative is a delete that looks like every other control on its row.
...
### 10.5 FAB
64 by 56dp pill in actionBlue with the add glyph in `card`, above the tab bar at the trailing edge, morphing to a 20dp corner on press. **Amended in the polish pass from `48dp circle`**: 48dp is exactly section 13's touch floor, so the app's most used control had no margin at all above the minimum, and a circle is the 2014 default rather than a decision. The wider pill shares the vocabulary the anchors, the chips and the tab bar already speak, and it gives A.4's shape law something to morph from. The glyph was white and measured 2.63:1 on the dark `actionBlue`, section 13; a filled action surface inverts its label to `card` in both worlds, 10.7, which is the inversion 10.8 already used for the destructive button.
...
**Appended with the archive view, issue #15, because section 10 described every surface in this app except this one.** 10.15's destination table has had a row for `Archived areas` since phase 2 and nothing said what was on it, so the glyph in the Areas header, 10.1, was drawn live and wired to nothing for eleven phases. `MASTER_BUILD_PROMPT.md` section 5 owns the behavior: archiving hides an area from Areas, from Momentum tiles and from widgets, its history stays, and the archive view "allows unarchiving or permanent deletion with a typed confirmation".

**What this screen is for.** Archiving is offered on the area detail sheet, one row above Delete, it is one tap, and it takes the area's whole queue with it. Without this screen that is a destructive act with no way back, which is the thing `CLAUDE.md` rule 12 refuses for an item and had not been asked about for an area. **This screen is the undo, and it does not expire.**

**It is a pushed screen**, 10.15: entered from the archive glyph, left by back, covering the tab it came from, and the floating tab bar is not drawn over it. The title is the serif `displayTitle` every pushed screen uses. The control that opens it reads `Archived areas` and the screen is titled `Archive`, and they differ on purpose: a button says where it goes and a title says where you are, and a button reading `Archive` alone is a verb with no object.

**A row is one card**, 10.3's card at rest with no wash, carrying the 7dp dot at the idle 45 percent, the area name, and a count of what is inside it.

- **The name takes the subject rank, `itemTitle` in inkPrimary, and on the Areas screen it does not.** 10.3 draws it small and in the area's own label color because there the subject is the item and the area is the context around it. Nothing is happening in an archived area, so there is no item to be the subject and the area is one
- **The count is active plus queued**, which is what comes back if it is restored. Completed items are history rather than contents, and counting them would make a long finished area look full
- **Nothing else.** No last active line, no date archived, no size, no age. `Last active 12 days ago` on this screen is the app telling somebody how long ago they gave up on something, which is an observation about their own data, and `MASTER_BUILD_PROMPT.md` section 11 allows exactly one path for those. A count query is a number; a reading of that number is a sentence, and this screen writes none

**Both actions are written out on the row, `Restore` and `Delete`.** The statistically common archive row is a tap that opens something, a trailing icon, or a swipe, and section 15 makes each of those a reason to look twice. All three are refused and the same fact refuses them: the person most likely to be on this screen archived something by accident and is looking for the way back. A label they can read beats a gesture they have to know, a glyph they have to interpret, and one more surface between them and their area. Swipe is refused twice over, because there is a delete on this row and a full swipe never commits one, 10.3.1.

`Restore` is a secondary button, 10.7, and `Delete` is that button's geometry with a transparent ground and the delete ink the detail sheet's own delete row takes. Restore is the weightier of the two because it is the reason the screen exists. Nothing rests on that weight: the delete is still two deliberate taps and a typed word away, through the same confirmation the Areas list uses.

**A row is not a door into the area, and the two controls are the whole of what it does.** There is no read-only detail sheet for an archived area, and the count is the summary. The way to look inside one is to restore it and open it where every other area is opened, which is one tap and is reversible in one more. A second surface that showed an area's queue would be a second place the queue is drawn, with its own rules about what a queued item offers when the area it is in cannot have an active item, and it would have to be kept in step with 10.3 forever to answer a question that is answered by putting the area back.

**Restoring puts the area back where it was**, not at the end of the list. The end is what almost every archive does and section 15 makes that a reason to look at it: for somebody undoing an accident it is a second thing to undo, because they get the area back and then have to remember where it sat and drag it there. Position is part of what an area is here, since the list is ordered by hand. The mechanism is that an archived area keeps its order key and no live area can be given it, so the round trip is free; `data/repo/AreaRestore.kt` carries the one case where a log this app did not write makes it a question.

**The archive can be emptied, and that is settled rather than open.** `MASTER_BUILD_PROMPT.md` section 5 puts permanent deletion on this screen and wins on behavior. It is also the better of the two: an archive nobody can empty grows forever, and a place a person stops putting things into because it is one way is a place that fails at the one job it has. What makes it safe is not the absence of the door but the length of the walk to it, and the typed word is that walk.

**The empty state is the ordinary state.** 10.13 makes an empty state an invitation that names the next action in plain words, and here there is no next action to name: an app that invited somebody to go and archive an area would be inviting them to put a part of their life away. The invitation is the reassurance instead. `Nothing archived`, and then that an area archived waits here with everything in its queue and can be brought back at any time, which is what a person opening an empty archive needs to read and is the sentence that would have saved the person who found issue #15. No button, and the back glyph the screen already carries is the way out.

**An empty archive and an archive that has not been read yet are two states, and only one of them says anything.** Until the list has arrived the screen draws its title and nothing under it. An empty state is a claim, and `Nothing archived` shown for a frame in front of somebody who came here to find an area is the one wrong sentence this screen could say. It is the distinction 10.15's zero areas case already makes on the Areas screen, where the empty state waits 150ms behind 8.2 item 25 for the same reason.
...
**Built, phase 8, and item 8 finished in phase 9b.** Every one of the nine items below is on the screen. The closing line's block was built and always empty, because a closing line is layer six and layer six was phase 9b. Layer six now composes, so the block draws a plan when there is one, both answers are live, and 8.2 item 26's settle runs.
...
**After an accept the block stops offering and starts stating. Phase 9b.** The nominal offer is replaced by the plan's stored first person line, the pill settles, the decline goes, and nothing else in the app changes. `CLARITY_LOGIC_ENGINE.md` 10.5: no toast, no celebration, no bounce, no haptic heavier than an ordinary tap, and no reminder, notification, badge, tile or home screen card afterwards. Declining writes no event at all and ignoring both is identical to declining, so there is no state a person can be left in by not answering. **The plan exists in this block and nowhere else**, and `PlanSurfaceTest` reads the app's own sources to keep that true, because it is the one rule on this surface that no type can enforce: the offense would be a new file somewhere else that reads a plan and draws it.

**Where the two options sit, which this section leaves open, and what phase 9b chose.** The statistically common answer is a row: two controls on one baseline, the dismissive one leading and the affirmative one trailing, which is what a card and a dialog do with a yes and a no everywhere. It is refused twice over. A row is a dialog footer, and a dialog footer is a question that holds the page until it is answered, while this is the one block in the app where not answering is a complete answer. And two things on one baseline read as a matched pair whatever their treatment, with the one carrying a ground winning the pair, so a row would take item 8's one deliberate asymmetry and put a second, louder one on top of it. **Stacked and centered on the page's own axis instead**, so the block reads as three things at the end of a page of centered prose: a sentence, one thing that can be done about it, and a way to close the subject. **And the air under the pill is the air above it**, 12dp of layout against 24dp, which is the same decision seen from below. The common answer groups the two controls within four to eight dp of one another and separates that group from the content, and the grouping is exactly what makes them read as a pair of buttons; the counter argument, that both are answers to the line above and belong together, loses to the same sentence, because a group of two reads as a question with a required answer. The two numbers differ because the decline's 48dp touch target already carries about 18dp of its own space above its text while the closing line leaves about 6dp of descent below its own, so both gaps draw as roughly 30dp. Section 15, and worth an owner's glance on the device like every optical number here.

**What a settled pill looks like, which this section also leaves open.** 8.2 item 26 gives the pill a fill from the center over 250ms, a label crossfading to a confirmation and a settle at reduced prominence, and it does not say what the fill fills to. **The front traveling out from the center is the reduced prominence arriving, not a brightening.** The obvious reading is that a pill which fills fills to a stronger gold, which is a flash of light at the moment somebody accepts, which is the celebration section 14 rules out, and which would need a peak value no document states. Filling to the settled value needs no second number and no second animation: one front leaves the center, what is behind it is the pill at rest, and nothing on this surface ever gets brighter than it already was. **The ground halves and the label does not.** Seven percent is fourteen halved, the one reduction that invents nothing, and it is the ground rather than the label because the ground is what made the pill a control; dimming both would be two reductions where 6.1's habit through this document is one, and it would make the sentence somebody has just chosen the hardest thing on the page to read. **A settled pill also stops describing itself as a control**, carrying no click handling, no focus ring and no button role, so it reaches TalkBack as the word it shows rather than as a disabled button. There is no un-accept, and a control that did nothing would be a lie about that. Section 15.

**The haptic on the accept is `planAccepted` and not `select`, corrected in phase 9b.** Section 9 gives that row `PRIMITIVE_TICK` at 0.5 and states the reason in the table itself, that accepting is not an achievement. Phase 8 wired the pill to `select`, which is `PRIMITIVE_CLICK` at 0.6 and is what a Pulse answer, a segmented choice and a theme tile fire. The difference is small in the hand and it is the difference between an app that acknowledges a tap and one that congratulates somebody, which is the whole argument of this block.
...
- **A semantic palette for the Trail's event glyphs: green completions, amber Pulses, red deletes.** 3.1 scopes `positiveGreen` to completion, `warnAmber` to the Pulse ready dot and `deleteMuted` to the delete action, one job each, so a second use of any of them is a color that means two things. 3.4 permits an area accent in four forms and none of them is a per event hue. Section 11 records what the icon column carries instead, which is a glyph and nothing else
