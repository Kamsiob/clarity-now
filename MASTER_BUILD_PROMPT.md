...
> **The four captions were rewritten in the polish pass and the Report's again for issue #66.** The three arguments and the audit that followed them are in `docs/ONBOARDING_VARIANTS.md`; the short version is that `an honest mirror` is a metaphor, `Your rhythm, without the guilt.` names a feeling the reader is assumed to have, and the three fragment triads were the shape group B rejected twice. The Report's caption then had one more job put on it. Of the five things this section says onboarding must convey, **that the app notices what actually happened and says it plainly, and never invents or judges**, had no carrier anywhere in the sequence, and the Report is the surface that does the noticing. It reads `Every Sunday, a page about your week. It notices what happened and says it plainly. It does not make anything up. Written on this phone, never sent anywhere.` The other half of that conveyance is one card above it: `It counts what you did. There is no score anywhere in this app.`
>
> **The queue is shown in beat 1 rather than asserted.** Its four demo cards carry the same `queue_waiting` line the real area card carries, so `3 waiting` under Work is the line a person meets on their own screen a few taps later, and it falls to `2 waiting` on the same fade the completed title crosses on. That was only possible once the real card said it, which is issue #65.
...
**Detection built in phase 3b. The two engine side consequences built in phase 6 and phase 8. The surface built in phase 12c**, issue #56. From Addendum 01 4d.

**Read that again before building anything near this section.** Phase 6 owned the surface and did not carry it, and it went four phases without one, which is why every part of it below is written out rather than summarized. What phase 6 built is the first of the pair of rules that follow the screen: the Pulse writes nothing for the first two days back, as step 2b of the sequence in 11.3. What phase 8 built afterward is the second: the week long withholding on the Report, the Momentum headline and the Areas banner. What phase 12c built is the screen: the two choices, the demotion by `ITEM_QUEUED`, the once per gap dismissal, and the Daylight treatment in `design-v3.md` 11.2.

The consequence that was worth stating plainly, and is now closed: for four phases the suppression rules existed without the screen they were written to protect, so nothing could greet a returning person with a measurement of their absence and nothing greeted them at all. Phase 10 built the cold start gate in `ui/onboarding/FirstRunGate.kt` and left the place for this route marked, after both flags, exactly where 10.15 puts it. Phase 12c filled it. **It is `ui/reentry/`, two files, and it is verified by three test classes rather than by looking at it**, because somebody who opens this app every day never has a gap and so nobody who builds or tests it will ever see this screen.
...
**How phase 12c holds that, since a sentence is not a gate.** The screen's four strings are fixed copy in `strings.xml`, which `design-v3.md` 11.2 settles and gives the reasoning for: an observation needs something to be about and this section leaves it nothing. `ReEntryLanguageTest` reads that file and the screen's own source and asserts that the four are the only strings the screen can reach, that none carries a digit, a month name, a word that measures an absence, a question mark or a placeholder, that no literal and no accessibility label carries a word the four do not, and that nothing in the source counts anything. `ReEntryRoutingTest` walks a log a day at a time and asserts the screen is offered on the day of a return and on no other day, never before onboarding is complete, never on a first ever open, and again for a second gap. `ReEntryClearingTest` asserts the second choice writes `ITEM_QUEUED` and nothing else, that every demoted item lands at the head of its own queue, and that the projection afterwards holds the same items, the same completions and no tombstone. **And the date never reaches the composition at all**: `ReEntryScreen` takes two callbacks and no values, so there is no number on the screen to render, to format or to subtract from.
...
**What it deliberately did not build:** design-v3.md 10.15's third cold start check, the re-entry route at fourteen days. It left the place for it marked, after both branches and never inside one, and **phase 12c built it there**, 14b.4 and 11.2.
...
- **Settings is hosted from inside the Areas tab**, so it did not cover the floating tab bar the way `design-v3.md` 10.15 makes a pushed screen cover it. **Closed by phase 12c**, and not by the remedy this line predicted: rather than moving pushed screens into `ClarityShell`, they register their depth in a composition local and the shell stops composing the bar while one is up. Same visible result, one line per screen, no navigation rewrite
...
- **The archived areas view did not land with this phase**, so for four phases the archive glyph in the Areas header was a live control wired to nothing, while archiving from the detail sheet was a one way tap that took an area's whole queue with it. **Built after phase 12c**, with restore, the typed permanent deletion and the `AREA_DELETED` tombstone. A restored area lands where it was rather than at the end, which turned out to cost nothing: every area key is chosen against the restorable set rather than the visible one, so an archived key is reserved while it is away
